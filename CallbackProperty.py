...
def make_callback(var_name, *callbacks, advanced_params=False):
    ...
    The variable var_name will become a property implementing callback in the setattr function.
    ...
    callback mechanism as you might cause exceptions.
    ...
    NOTE: You should initialize the variable in the __init__ function. The first invocation of the setter method is used to
    initialize the callback data structure and no callback will be invoked. Not initializing the variable may cause
    unexpected behavior

    :param var_name: The name of the variable.
    :type var_name: str
    :param callbacks: Callbacks to initialize the variable with.
    :type callbacks: func(any)
    ...
    def add_callback(self, callback, *callbacks):
        getattr(self, '_' + var_name).add_callback(callback)
        for cb in callbacks:
            getattr(self, '_' + var_name).add_callback(cb)
    add_callback.__name__ = var_name + '_add_callback'
    add_callback.__doc__ = """Add callback to variable '{0}'.
    ...
    def remove_callback(self, callback, *callbacks):
        getattr(self, '_' + var_name).remove_callback(callback)
        for cb in callbacks:
            getattr(self, '_' + var_name).remove_callback(cb)
    remove_callback.__name__ = var_name + '_remove_callback'
    remove_callback.__doc__ = """Try to remove a callback from the variable '{0}'. Raises ValueError if the callback
    is not present.
