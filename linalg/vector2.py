...
class Vector2(Sequence):
    ...
    def dot(self, other):
        return self.x * other.x + self.y * other[1]
    ...
    def __add__(self, other):
        if Vector2._test_type(self, other):
    ...
    def __pow__(self, power, modulo=None):
        if Vector2._test_type(self, power):
            if modulo:
                if Vector2._test_type(self, modulo):
                    return Vector2(self.x ** power[0] % modulo[0], self.y ** power[1] % modulo[1])
                else:
                    return Vector2(self.x ** power[0] % modulo, self.y ** power[1] % modulo)
            else:
...
if __name__ == '__main__':
    print('Testing Vector2 class... ', end='')
    ...
    # From polar
    from linalg import pi
    v19 = Vector2.from_polar(1, pi/2)
    v20 = Vector2.from_polar(2, pi)
